...
const countNumberOfElements = (arr) => {
    let myVar = arr.reduce(function (accumulator, value, index){
      accumulator+=1;
      return accumulator;
    }, 0);
    return myVar;
...
Note: You must use reduce for this challenge. You may not use the built-in .reverse() string method. arr = string
------------------------------------------------------------------------------------------------ */

const reversedString = (theStr) => {
    let arr = theStr.split('');
    let newArr = arr.reduce((str,val,idx, arr) => {
          str = val + str;
        return str;
      }, '');
      return newArr;
...
const countNumberOfChildren = (arr) => {
    let newNum = arr.reduce((num,val,idx,arr) => {
...
const calculateAverage = (arr) => {
  let average = arr.reduce((avg,val,idx)=>{
    avg +=val;
    if(idx+1 === arr.length){
      avg = avg / (idx+1);
    }
...
const countPrimeNumbers = (arr) => {
    let numPrime = arr.reduce((num, val, idx)=>{
      if(isPrime(val)){
        num+=1;
      }
      return num;
...
const extractStat = (statName, arr) => {  
    let statObject =  arr.reduce((object,obj,idx)=>{
       console.log(obj.stat.name, statName);
