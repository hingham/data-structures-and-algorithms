...
const totalSum = (input) => {
  let theSum = input.reduce((acc,val,idx, arr)=>{
    acc+= arr[idx].reduce((sum, val, idx)=>{
      sum = sum + val;
      return sum;
    }, 0);
    return acc;
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = input.map((val,idx)=>{
    idx.length>1;
    let innerArry = val.filter((num,idx)=>{
      if(typeof(num)==='number'){
        return num%5===0   
      } 
    })
    var pow = innerArry.map((val,idx)=>{
...
let findShortest = (data) => {
  let compare = data[0].height;
  let name = '';
  console.log('compare' + compare);
  return data.reduce((cum, num, idx)=>{
    console.log(num.height);
    
   if(num.height > compare){
     compare = num.height;
     name = num.name;
   }
   console.log(compare);
