...
const validatePin = (pin) => {
  pin = pin.toString()+ ' ';
  let regex = /\d{4}\D/;
  return regex.test(pin.slice(0,5));
};
...
const validateEmail = (email) => {
    let regex = /\w+\s?\.?\s?\w+?\@\w+?((.net)|(.com)|(.org))\b/;
...
const findTagNames = elements => {
    let regex = /\<(\/\D{1,4}\d?)\>/g;
    let regexArr = elements.join().match(regex);
    return regexArr.map((endTag)=>{
      return endTag.slice(1, endTag.length-1);
    });
  }
